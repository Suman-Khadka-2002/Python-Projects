...
def deposit():
    while True:
        ...
        if amount.isdigit():    # checks if the entered amount is digit or not
            amount = int(amount)
            if amount > 0:
                break
            else:
...
def get_number_of_lines():
    while True:
        ...
        if lines.isdigit():    # checks if the entered amount is digit or not
            lines = int(lines)
...
def spin(balance):
    ...
    print(f"You are betting Rs.{bet} on {lines} lines. Total bet = {total_bet}")
